  ...
  return (
    <div className="grid md:grid-cols-4 gap-4">
      {Array.from({ length: 8 }).map((_, i) => (
        <div key={i} className="animate-pulse rounded-lg bg-gray-200 dark:bg-gray-700 h-80" />
      ))}
    </div>
  );
};
...
const Books = () => {
  ...
  useEffect(() => {
    // Fake loading delay so skeletons show before content
  ...
  const getBooksForDisplay = (category: string) => {
    ...
    return getBooksByCategory(actualCategory).filter(
      (book) =>
        selectedAge === "All Ages" ||
        book.ageRange.includes(selectedAge.split(" ")[1])
    );
  };
